...
/*
 * Dont touch clk related for now until we complete clk driver
 */
//#define USING_CLK_CODE

static u8 meson_nfc_read_byte(struct nand_chip *chip);
...
#define NFC_CMD_SCRAMBLER_ENABLE	1
#define NFC_CMD_SCRAMBLER_BIT		BIT(19)
...
#define NAND_CE2		(0xb << 10)
#define NAND_CE3		(0x7 << 10)
...
NAND_ECC_CAPS_SINGLE(meson_8726m3_ecc_caps,
...
static void nand_chip_pinmux(struct meson_nfc *nfc)
{
        u32 val;
        // pullup
        volatile u32* addr = (u32*)nfc->pinmux_reg_base;
#if 0
        val = *(addr+3);
        val |= (0xff|1<<16);
        *(addr+3) = val;
        ...
        // pinmux
        //addr= (u32*)ioremap(0xc11080b0,52);
#if 0
        val = *(addr+5);
        val |= 0x7<<7;
        *(addr+5)=val;
        ...
        *(addr+3)=val;
        ...
        //val=*(addr+2);
	val = readl(addr+2);
        val|=(1<<27|1<<26|1<<25|0xf<<18|1<<17);// need rb pin
        //val|=(1<<25|1<<17);// ce0 and rb0
        //*(addr+2)=val;
	writel(val, addr+2);
}
static void nand_chip_unpinmux(struct meson_nfc *nfc)
{
        u32 val;
        ...
        // pinmux
        addr= (u32*)nfc->pinmux_reg_base;

        //val=*(addr+2);
        //val&=~(1<<27|1<<26|1<<25|0xf<<18);
        //val&=~(1<<25|1<<17);
        //*(addr+2)=val;
...
static void meson_nfc_select_chip(struct nand_chip *nand, int chip)
{
	...
	// for 8726M3 need set pinmux before doing something
	// Why?? No manual just from leaked uboot code
	nand_chip_pinmux(nfc);

	chip_index = meson_chip->sels[chip];
	switch(chip_index)
	{
		case 0:
			nfc->param.chip_select = NAND_CE0;
			break;
		case 1:
			nfc->param.chip_select = NAND_CE1;
			break;
		case 2:
			nfc->param.chip_select = NAND_CE2;
			break;
		case 3:
			nfc->param.chip_select = NAND_CE3;
			break;
	}
...
extern unsigned long long jffies;
...
static int meson_nfc_read_buf(struct nand_chip *nand, u8 *buf, int len)
{
	...
	cmd = NFC_CMD_N2M | (len & GENMASK(13, 0));//0x3ffff
#endif
	writel(cmd, nfc->reg_base + NFC_REG_CMD);
...
#else
	cmd = NFC_CMD_M2N | (len & GENMASK(13, 0)); // 0x3fff
#endif
	writel(cmd, nfc->reg_base + NFC_REG_CMD);
				    ...
				    int page, int raw, int seed)
{
	...
	if(meson_chip->two_plane_mode)
		data_len =  mtd->writesize/2 + mtd->oobsize/2;
	else
				    ...
				    int oob_required, int page)
{
	...
	int ret = 0;
	int i;
	int page_in_plane = page / 2;// determine which page in the plane
	int blk_in_plane = (page/2/256);
	int page_in_blk = (page/2)%256;
	
	// fill buffers
	for (i = 0; i < 2; i++) {
		if (buf)
			meson_nfc_set_data_oob(nand, buf+i*mtd->writesize/2, oob_buf+i*mtd->oobsize/2);
		else
			meson_nfc_set_data_oob(nand, NULL, oob_buf+i*mtd->oobsize/2);
	}

	for (i = 0; i < 2; i++) {
		if (i == 0)
			ret =  meson_nfc_write_page_sub(nand, blk_in_plane*2*256+page_in_blk, 1, page);
		else
			ret =  meson_nfc_write_page_sub(nand, (blk_in_plane*2+1)*256+page_in_blk, 1, page);
		if (ret)
			return ret;
	}
	return ret;
				      ...
				      const u8 *buf, int oob_required, int page)
{
	...
	for (i = 0; i < 2; i++) {
		...
		if (buf)
			...
			memset(meson_chip->data_buf, 0xff, mtd->writesize);
		memset(meson_chip->info_buf, 0, nand->ecc.steps * PER_INFO_BYTE);
		meson_nfc_set_user_byte(nand, oob_buf);
			
		if (i == 0)
			ret = meson_nfc_write_page_sub(nand, 
					blk_in_plane*2*256+page_in_blk, 
					0, page);
		else
			ret = meson_nfc_write_page_sub(nand, 
					(blk_in_plane*2+1)*256+page_in_blk, 
					0, page);
		if (ret)
			return ret;
	}
				   ...
				   int oob_required, int page)
{
	...
	for (i = 0; i < 2; i++) {
		if (i == 0)
			ret = meson_nfc_read_page_sub(nand, page/2, 1, page);
		else
			ret = meson_nfc_read_page_sub(nand, page/2+0x100, 1, page);
		if (ret)
			return ret;
				     ...
				     int oob_required, int page)
{
	...
	// calculate two plane page address
	for (plane_index = 0; plane_index < 2; plane_index++) {
		// do read page
		if (plane_index == 0)
			ret = meson_nfc_read_page_sub(nand, 
					blk_in_plane*2*256+page_in_blk, 
					0, page);
		else
			ret = meson_nfc_read_page_sub(nand, 
		...
		if (ret == ECC_CHECK_RETURN_FF) {
			if (buf) {
				if(meson_chip->two_plane_mode)
					memset(buf, 0xff, (mtd->writesize/2));
				else
					memset(buf, 0xff, mtd->writesize);
			}
			if(meson_chip->two_plane_mode)
				memset(oob_buf, 0xff, (mtd->oobsize/2));
			else
			...
			if(meson_chip->two_plane_mode)
				memcpy(buf, meson_chip->data_buf, mtd->writesize/2);
			else
				memcpy(buf, meson_chip->data_buf, mtd->writesize);
		}

		/*
		 * read_oob->read_page_hwecc buf will set to null
		 */
		if(buf)
			buf += mtd->writesize/2;
		oob_buf += mtd->oobsize/2;
...
{
	return meson_nfc_write_page_raw(nand, NULL, 1, page);
}

static int meson_nfc_write_oob(struct nand_chip *nand, int page)
{
	return meson_nfc_write_page_hwecc(nand, NULL, 1, page);
}
...
meson_nand_op_get_dma_safe_input_buf(const struct nand_op_instr *instr)
{
	...
	ret = kzalloc(instr->ctx.data.len, GFP_KERNEL);
	return ret;
}
...
meson_nand_op_get_dma_safe_output_buf(const struct nand_op_instr *instr)
{
	...
	ret = kmemdup(instr->ctx.data.buf.out,
...
static int meson_chip_buffer_init(struct nand_chip *nand)
{
	...
	if(meson_chip->two_plane_mode)
		nsectors = (mtd->writesize / 2) / nand->ecc.size;
	else
		nsectors = mtd->writesize / nand->ecc.size;

	if(meson_chip->two_plane_mode)
		page_bytes =  (mtd->writesize / 2) + (mtd->oobsize / 2);
	else
...
static int meson_nand_attach_chip(struct nand_chip *nand)
{
	...
	nand->options |= NAND_NEED_SCRAMBLING;// enable scrambling
	...
	meson_chip->two_plane_mode = 1;/* use two plane mode to be compatible with uboot in the device */
	if(meson_chip->two_plane_mode) {
		mtd->writesize *= 2;
		mtd->oobsize *=2;
		mtd->erasesize *=2;
...
static u8 meson_nfc_read_byte(struct nand_chip *chip)
{
	...
	//DRD
	writel(0x8<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	//IDLE
	writel(0xc<<14|0xe<<10|0x5,nfc->reg_base+NFC_REG_CMD);

	meson_nfc_wait_cmd_finish(nfc, 1000);

	return (u8)readl(nfc->reg_base+NFC_REG_BUF);
}
static void meson_nfc_command(struct nand_chip *chip, unsigned int command,
		int column, int page)
{
	struct meson_nfc *nfc = nand_get_controller_data(chip);
	...
	switch(command)
	{
		case NAND_CMD_PARAM:
			//cmd
			writel(0x5<<14|0xe<<10|NAND_CMD_PARAM,nfc->reg_base+NFC_REG_CMD);
			writel(0xc<<14|0xe<<10|0,nfc->reg_base+NFC_REG_CMD);
			//addr
			writel(0x6<<14|0xe<<10|0,nfc->reg_base+NFC_REG_CMD);
			writel(0xc<<14|0xe<<10|5,nfc->reg_base+NFC_REG_CMD);
...
static void m3_select_chip(struct nand_chip *chip, int chipnr)
{
	struct meson_nfc *nfc = nand_get_controller_data(chip);
...
#if 0 /* for old interface */
	nand->legacy.read_byte = meson_nfc_read_byte;
	nand->legacy.cmdfunc = meson_nfc_command;
	nand->legacy.select_chip = m3_select_chip;
#endif
...
#endif
static const struct meson_nfc_data meson_8726m3_data = {
	.ecc_caps = &meson_8726m3_ecc_caps,
};

static const struct of_device_id meson_nfc_id_table[] = {
	{
		.compatible = "amlogic,meson-8726m3-nfc",
		.data = &meson_8726m3_data,
	}, 
	{}
};
...
static u8 dma_buf[4096];
static u8 info_buf[4096];
static void rawDrvTest(struct meson_nfc *nfc)
{
	int i;
	u8 v;
	u8 buf[256];// ugly code
	u32 addr;//data buffer
	u32 iaddr;//info buffer

#if 0
	//select chip
	nand_chip_pinmux();
	//idle cycle
	writel(0xc<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	// reset
	writel(5<<14|0xe<<10|0xff,nfc->reg_base+NFC_REG_CMD);
	msleep(50);
#endif
#if 1 // read parameter page command
	nand_chip_pinmux(nfc);
	//idle cycle
	writel(0xc<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	// cmd cycle 
	writel(5<<14|0xe<<10|0xec,nfc->reg_base+NFC_REG_CMD);
	//writel(5<<14|0xe<<10|0x90,nfc->reg_base+NFC_REG_CMD);
	writel(0xc<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	// addr cycle 
	writel(6<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	writel(0xc<<14|0xe<<10|0x5,nfc->reg_base+NFC_REG_CMD);
	msleep(1);
#endif
#if 0 // read page command
	nand_chip_pinmux();
	//idle cycle
	writel(0xc<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	// cmd cycle 
	//writel(5<<14|0xe<<10|0xec,nfc->reg_base+NFC_REG_CMD);
	writel(5<<14|0xe<<10|0x00,nfc->reg_base+NFC_REG_CMD);
	// addr cycle 
	writel(6<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	writel(6<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	writel(6<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	...
	writel(6<<14|0xe<<10|0x0,nfc->reg_base+NFC_REG_CMD);
	// cmd cycle
	writel(5<<14|0xe<<10|0x30,nfc->reg_base+NFC_REG_CMD);
	msleep(1);
#endif
	...
	for(i=0;i<64;i++) {
		//DRD cycle
		writel(8<<14|0xe<<10|0,nfc->reg_base+NFC_REG_CMD);
		msleep(1);
		v = (u8)readl(nfc->reg_base+NFC_REG_BUF);
		printk("000 %02x ",v);
		msleep(1);
...
#else
 // DMA
 	memset(info_buf,0,4096);
	addr = dma_map_single(nfc->dev, dma_buf, 4096, DMA_FROM_DEVICE);
	iaddr = dma_map_single(nfc->dev, info_buf, 4096, DMA_FROM_DEVICE);
...
#endif
	//ADL
	writel((0<<16|0x3<<20|addr&0xffff),nfc->reg_base+NFC_REG_CMD);
	//ADH
	writel((1<<16|0x3<<20|(addr>>16)&0xffff),nfc->reg_base+NFC_REG_CMD);
	//AIL
	writel((2<<16|0x3<<20|iaddr&0xffff),nfc->reg_base+NFC_REG_CMD);
	//AIH
	writel((3<<16|0x3<<20|(iaddr>>16)&0xffff),nfc->reg_base+NFC_REG_CMD);
	//N2M
	writel((1<<17|2<<20|16),nfc->reg_base+NFC_REG_CMD);
	//msleep(1);
	// no ran mode
	//writel((1<<17|2<<20|2|5<<14|4),nfc->reg_base+NFC_REG_CMD);
	...
	dma_unmap_single(nfc->dev, addr, 4096, DMA_FROM_DEVICE);
	dma_unmap_single(nfc->dev, iaddr, 4096, DMA_FROM_DEVICE);

	for(i=0;i<16;i++)
		printk("111 %02x ",dma_buf[i]);
...
static int meson_nfc_probe(struct platform_device *pdev)
{
	...
	nfc->pinmux_reg_base = ioremap(0xc11080b0,52);
	if (IS_ERR_OR_NULL(nfc->pinmux_reg_base))
		return PTR_ERR(nfc->pinmux_reg_base);
...
#if 0/* setting cfg until we know what timing is it*/
	printk("meson_nfc_probe 555\n");
	writel(0xA3, nfc->reg_base + NFC_REG_CFG);
#endif
