 ...
 *
 * Copyright (C) 2020 Su Yanjun <suyanjun218@gmail.com>
 */
...
#define AML_UART_REG5			0x14 /* 8726M3 dont use */
...
static void meson_uart_change_speed(struct uart_port *port, unsigned long baud)
{
	...
	while (!meson_uart_tx_empty(port))
		cpu_relax();
#if 0/* compatible with 8726M3 ? No manual!! */
...
#endif
#if 0 /* dont enable speed change until clk driver ok */
	/*
	 * baud setting in:
	 * CONTROL [11:0] low 12 bits
	 * MISC [19:26] high 8 bits
	 */
	val = ((port->uartclk * 10 / (baud * 4) + 5) / 10) - 1;
	if ((val>>12)&0xf) {
		tmp = readl(port->membase + AML_UART_MISC);
		writel(((tmp & 0xff0fffff) | (val <<20)), port->membase + AML_UART_MISC);
	}

	tmp = readl(port->membase + AML_UART_CONTROL);
	writel(((tmp&~0xfff)|(val&0xfff)), port->membase + AML_UART_CONTROL);
#endif
}
...
#ifdef CONFIG_SERIAL_MESON3_CONSOLE
...
{
	printascii("meson_serial_early_console_setup 000\r\n");
	if (!device->port.membase)
		return -ENODEV;

	printascii("meson_serial_early_console_setup 111\r\n");
	meson_uart_enable_tx_engine(&device->port);
	printascii("meson_serial_early_console_setup 222\r\n");
	device->con->write = meson_serial_early_console_write;
	printascii("meson_serial_early_console_setup 333\r\n");
	return 0;
}
/* Stable bindings */
OF_EARLYCON_DECLARE(meson, "amlogic,meson3-uart",
...
static int meson_uart_probe(struct platform_device *pdev)
{
	...
	printascii("meson_uart_probe 000\r\n");
	if (pdev->dev.of_node)
		pdev->id = of_alias_get_id(pdev->dev.of_node, "serial");

	printascii("meson_uart_probe 111\r\n");
	if (pdev->id < 0) {
	...
	/* dont use clk driver until we port clk driver ok
	 * uartclk ???
	 */
#if 0
	/* Use legacy way until all platforms switch to new bindings */
	if (of_device_is_compatible(pdev->dev.of_node, "amlogic,meson3-uart"))
	...
	port->has_sysrq = IS_ENABLED(CONFIG_SERIAL_MESON3_CONSOLE);
	port->dev = &pdev->dev;
	port->line = pdev->id;
	port->type = PORT_MESON;
	port->x_char = 0;
	...
	printascii("meson_uart_probe 222\r\n");
	if (meson_uart_request_port(port) >= 0) {
		meson_uart_reset(port);
		meson_uart_release_port(port);
	}

	printascii("meson_uart_probe 333\r\n");
	ret = uart_add_one_port(&meson_uart_driver, port);
	if (ret)
		meson_ports[pdev->id] = NULL;

	printascii("meson_uart_probe 444\r\n");
	return ret;
}
...
static int __init meson_uart_init(void)
{
	...
	printascii("meson_uart_init 000\r\n");
	ret = meson_serial_console_init();
	if (ret)
		return ret;
	
	printascii("meson_uart_init 111\r\n");
	ret = uart_register_driver(&meson_uart_driver);
	if (ret)
		return ret;

	printascii("meson_uart_init 222\r\n");
	ret = platform_driver_register(&meson_uart_platform_driver);
	if (ret)
		uart_unregister_driver(&meson_uart_driver);

	printascii("meson_uart_init 333\r\n");
	return ret;
}
